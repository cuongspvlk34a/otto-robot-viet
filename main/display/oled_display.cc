...
OledDisplay::OledDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
    ...
    ESP_LOGI(TAG, "Thêm màn hình OLED");
    ...
    if (display_ == nullptr) {
        ESP_LOGE(TAG, "Không thể thêm màn hình");
        return;
    }

    // Lưu ý: SetupUI() phải được gọi từ Application::Initialize(), không phải trong constructor,
    // để đảm bảo các đối tượng LVGL được tạo sau khi màn hình khởi tạo hoàn tất.
}

void OledDisplay::SetupUI() {
    // Ngăn gọi trùng lặp - nếu đã được gọi trước đó thì thoát sớm
    if (setup_ui_called_) {
        ESP_LOGW(TAG, "SetupUI() được gọi nhiều lần, bỏ qua lần gọi trùng lặp");
        return;
    }
...
void OledDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    // Thay thế tất cả ký tự xuống dòng bằng dấu cách
...
void OledDisplay::SetupUI_128x64() {
    ...
    /* Lớp 1: Thanh trên cùng - chứa biểu tượng trạng thái */
    ...
    /* Lớp 2: Thanh trạng thái - chứa nhãn văn bản ở giữa */
    ...
    lv_obj_align(status_bar_, LV_ALIGN_TOP_MID, 0, 0);  // Chồng lên top_bar_
    ...
    // Bắt đầu cuộn phụ đề sau một khoảng trễ
    static lv_anim_t a;
    lv_anim_init(&a);
