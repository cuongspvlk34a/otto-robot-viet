...
void AudioService::Initialize(AudioCodec* codec) {
    ...
    if (opus_decoder_ == nullptr) {
        ESP_LOGE(TAG, "Không thể tạo bộ giải mã âm thanh, mã lỗi: %d", ret);
    } else {
    ...
    if (opus_encoder_ == nullptr) {
        ESP_LOGE(TAG, "Không thể tạo bộ mã hóa âm thanh, mã lỗi: %d", ret);
    } else {
    ...
    if (codec->input_sample_rate() != 16000) {
        ...
        if (input_resampler_ == nullptr) {
            ESP_LOGE(TAG, "Không thể tạo bộ lấy mẫu lại đầu vào, mã lỗi: %d", resampler_ret);
        }
    }
...
void AudioService::Start() {
...
#if CONFIG_USE_AUDIO_PROCESSOR
    /* Khởi động task đầu vào âm thanh */
    ...
    /* Khởi động task đầu ra âm thanh */
...
bool AudioService::ReadAudioData(std::vector<int16_t>& data, int sample_rate, int samples) {
    ...
    /* Cập nhật thời điểm đầu vào gần nhất */
...
void AudioService::AudioInputTask() {
    while (true) {
        ...
        /* Dùng để kiểm tra âm thanh trong chế độ NetworkConfiguring khi nhấn nút BOOT */
        if (bits & AS_EVENT_AUDIO_TESTING_RUNNING) {
            if (audio_testing_queue_.size() >= AUDIO_TESTING_MAX_DURATION_MS / OPUS_FRAME_DURATION_MS) {
                ESP_LOGW(TAG, "Hàng đợi kiểm tra âm thanh đầy, dừng kiểm tra âm thanh");
            ...
            if (ReadAudioData(data, 16000, samples)) {
                // Nếu số kênh đầu vào là 2, chỉ lấy dữ liệu kênh trái
        ...
        /* Nạp dữ liệu cho wake word và/hoặc bộ xử lý âm thanh */
        ...
        // Lỗi timeout/đọc không được kết thúc task đầu vào.
        vTaskDelay(pdMS_TO_TICKS(10));
    }

    ESP_LOGW(TAG, "Task đầu vào âm thanh đã dừng");
}
...
void AudioService::AudioOutputTask() {
    while (true) {
        ...
        /* Cập nhật thời điểm đầu ra gần nhất */
        last_output_time_ = std::chrono::steady_clock::now();
        debug_statistics_.playback_count++;

#if CONFIG_USE_SERVER_AEC
        /* Ghi lại timestamp cho server AEC */
        if (task->timestamp > 0) {
            lock.lock();
    ...
    ESP_LOGW(TAG, "Task đầu ra âm thanh đã dừng");
}
...
void AudioService::OpusCodecTask() {
    while (true) {
        ...
        /* Giải mã âm thanh từ hàng đợi decode */
        if (!audio_decode_queue_.empty() && audio_playback_queue_.size() < MAX_PLAYBACK_TASKS_IN_QUEUE) {
            ...
            if (opus_decoder_ != nullptr) {
                ...
                if (ret == ESP_AUDIO_ERR_OK) {
                    ...
                    ESP_LOGE(TAG, "Không thể giải mã âm thanh sau khi resize, mã lỗi: %d", ret);
                    lock.lock();
                }
            } else {
                ESP_LOGE(TAG, "Bộ giải mã âm thanh chưa được cấu hình");
                lock.lock();
            }
            debug_statistics_.decode_count++;
        }
        /* Mã hóa âm thanh vào hàng đợi gửi */
        if (!audio_encode_queue_.empty() && audio_send_queue_.size() < MAX_SEND_PACKETS_IN_QUEUE) {
            ...
            if (opus_encoder_ != nullptr && task->pcm.size() == encoder_frame_size_) {
                ...
                if (ret == ESP_AUDIO_ERR_OK) {
                    ...
                    ESP_LOGE(TAG, "Không thể mã hóa âm thanh, mã lỗi: %d", ret);
                }
            } else {
                ESP_LOGE(TAG, "Không thể mã hóa âm thanh: bộ mã hóa chưa cấu hình hoặc kích thước frame không hợp lệ (nhận %u, kỳ vọng %u)",
    ...
    ESP_LOGW(TAG, "Task Opus codec đã dừng");
}
...
void AudioService::SetDecodeSampleRate(int sample_rate, int frame_duration) {
    ...
    if (decoder_sample_rate_ != codec->output_sample_rate()) {
        ESP_LOGI(TAG, "Đang lấy mẫu lại âm thanh từ %d sang %d", decoder_sample_rate_, codec->output_sample_rate());
        if (output_resampler_ != nullptr) {
            esp_ae_rate_cvt_close(output_resampler_);
        ...
        if (output_resampler_ == nullptr) {
            ESP_LOGE(TAG, "Không thể tạo bộ lấy mẫu lại đầu ra, mã lỗi: %d", resampler_ret);
        }
    }
}
...
void AudioService::PushTaskToEncodeQueue(AudioTaskType type, std::vector<int16_t>&& pcm) {
    ...
    /* Đẩy task vào hàng đợi mã hóa */
    std::unique_lock<std::mutex> lock(audio_queue_mutex_);

    /* Nếu task gửi vào hàng đợi gửi, cần gán timestamp */
    if (type == kAudioTaskTypeEncodeToSendQueue && !timestamp_queue_.empty()) {
        if (timestamp_queue_.size() <= MAX_TIMESTAMPS_IN_QUEUE) {
            task->timestamp = timestamp_queue_.front();
        } else {
            ESP_LOGW(TAG, "Hàng đợi timestamp (%u) đầy, bỏ qua timestamp", timestamp_queue_.size());
        }
...
void AudioService::EnableWakeWordDetection(bool enable) {
    ...
    ESP_LOGD(TAG, "%s phát hiện wake word", enable ? "Đang bật" : "Đang tắt");
    if (enable) {
        if (!wake_word_initialized_) {
            if (!wake_word_->Initialize(codec_, models_list_)) {
                ESP_LOGE(TAG, "Không thể khởi tạo wake word");
                return;
            }
            wake_word_initialized_ = true;
        }
        // Đặt lại bộ lấy mẫu lại đầu vào để xóa dữ liệu cache từ chế độ trước (ví dụ: AudioProcessor)
        // Tránh tràn buffer khi chuyển đổi giữa các kích thước feed khác nhau
        {
            std::lock_guard<std::mutex> lock(input_resampler_mutex_);
            if (input_resampler_ != nullptr) {
...
void AudioService::EnableVoiceProcessing(bool enable) {
    ESP_LOGD(TAG, "%s xử lý giọng nói", enable ? "Đang bật" : "Đang tắt");
    if (enable) {
        ...
        /* Đảm bảo không có âm thanh đang phát */
        ResetDecoder();
        audio_input_need_warmup_ = true;
        // Đặt lại bộ lấy mẫu lại đầu vào để xóa dữ liệu cache từ chế độ trước (ví dụ: WakeWord)
        // Tránh tràn buffer khi chuyển đổi giữa các kích thước feed khác nhau
        {
            std::lock_guard<std::mutex> lock(input_resampler_mutex_);
            if (input_resampler_ != nullptr) {
...
void AudioService::EnableAudioTesting(bool enable) {
    ESP_LOGI(TAG, "%s kiểm tra âm thanh", enable ? "Đang bật" : "Đang tắt");
    if (enable) {
        xEventGroupSetBits(event_group_, AS_EVENT_AUDIO_TESTING_RUNNING);
    } else {
        xEventGroupClearBits(event_group_, AS_EVENT_AUDIO_TESTING_RUNNING);
        /* Sao chép audio_testing_queue_ vào audio_decode_queue_ */
        std::lock_guard<std::mutex> lock(audio_queue_mutex_);
...
void AudioService::EnableDeviceAec(bool enable) {
    ESP_LOGI(TAG, "%s AEC thiết bị", enable ? "Đang bật" : "Đang tắt");
    if (!audio_processor_initialized_) {
        audio_processor_->Initialize(codec_, OPUS_FRAME_DURATION_MS, models_list_);
        audio_processor_initialized_ = true;
    }
...
void AudioService::CheckAndUpdateAudioPowerState() {
    ...
    if (output_elapsed > AUDIO_POWER_TIMEOUT_MS && codec_->output_enabled()) {
        // Giữ xung clock TX khi RX duplex đang hoạt động; nếu không RX có thể bị treo trên một số bo mạch.
