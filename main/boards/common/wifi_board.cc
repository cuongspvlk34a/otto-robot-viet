...
void WifiBoard::StartNetwork() {
    ...
    // Đặt callback sự kiện thống nhất — chuyển tiếp sang NetworkEvent kèm dữ liệu SSID
    ...
    // Thử kết nối hoặc vào chế độ cấu hình
    TryWifiConnect();
}
...
void WifiBoard::TryWifiConnect() {
    ...
    if (have_ssid) {
        // Bắt đầu thử kết nối kèm timer timeout
        ESP_LOGI(TAG, "Bắt đầu kết nối WiFi");
        esp_timer_start_once(connect_timer_, CONNECT_TIMEOUT_SEC * 1000000ULL);
        WifiManager::GetInstance().StartStation();
    } else {
        // Chưa có SSID nào được cấu hình, vào chế độ cấu hình
        // Chờ hiển thị phiên bản board xong
...
void WifiBoard::OnNetworkEvent(NetworkEvent event, const std::string& data) {
    switch (event) {
        case NetworkEvent::Connected:
            ...
            esp_timer_stop(connect_timer_);
#ifdef CONFIG_USE_ESP_BLUFI_WIFI_PROVISIONING
            // Đảm bảo tài nguyên blufi đã được giải phóng
            Blufi::GetInstance().deinit();
#endif
            in_config_mode_ = false;
            ESP_LOGI(TAG, "Đã kết nối WiFi: %s", data.c_str());
            break;
        case NetworkEvent::Scanning:
            ESP_LOGI(TAG, "Đang quét WiFi");
            break;
        case NetworkEvent::Connecting:
            ESP_LOGI(TAG, "Đang kết nối WiFi tới %s", data.c_str());
            break;
        case NetworkEvent::Disconnected:
            ESP_LOGW(TAG, "WiFi đã ngắt kết nối");
            break;
        case NetworkEvent::WifiConfigModeEnter:
            ESP_LOGI(TAG, "Đã vào chế độ cấu hình WiFi");
            in_config_mode_ = true;
            break;
        case NetworkEvent::WifiConfigModeExit:
            ESP_LOGI(TAG, "Đã thoát chế độ cấu hình WiFi");
            in_config_mode_ = false;
            // Thử kết nối lại với thông tin xác thực mới
            TryWifiConnect();
            break;
        default:
            break;
    ...
    // Gọi callback bên ngoài nếu đã được đặt
...
void WifiBoard::OnWifiConnectTimeout(void* arg) {
    ...
    ESP_LOGW(TAG, "Hết thời gian kết nối WiFi, chuyển sang chế độ cấu hình");
...
void WifiBoard::StartWifiConfigMode() {
    in_config_mode_ = true;
    // Chuyển sang trạng thái đang cấu hình WiFi
    ...
    // Hiển thị hướng dẫn cấu hình sau một khoảng trễ ngắn
    ...
    // Khởi động task cấp phát WiFi bằng âm thanh
    auto codec = Board::GetInstance().GetAudioCodec();
    int channel = codec ? codec->input_channels() : 1;
    ESP_LOGI(TAG, "Bắt đầu cấp phát WiFi bằng âm thanh, kênh: %d", channel);
...
void WifiBoard::EnterWifiConfigMode() {
    ESP_LOGI(TAG, "Gọi EnterWifiConfigMode");
    ...
    if (state == kDeviceStateSpeaking || state == kDeviceStateListening || state == kDeviceStateIdle) {
        // Đặt lại giao thức (đóng kênh audio, reset protocol)
        Application::GetInstance().ResetProtocol();

        xTaskCreate([](void* arg) {
            auto* board = static_cast<WifiBoard*>(arg);

            // Chờ 1 giây để quá trình phát âm thanh kết thúc
            vTaskDelay(pdMS_TO_TICKS(1000));

            // Dừng mọi kết nối đang thực hiện
    ...
    if (state != kDeviceStateStarting) {
        ESP_LOGE(TAG, "Gọi EnterWifiConfigMode nhưng trạng thái thiết bị không hợp lệ, trạng thái: %d", state);
        return;
    }
