...
private:
    ...
    bool DetectHardwareVersion() {
        ...
        for (size_t i = 0; i < sizeof(camera_addresses); i++) {
            ...
            if (ret == ESP_OK) {
                ...
                if (ret == ESP_OK && (data[0] != 0 || data[1] != 0)) {
                    ...
                    ESP_LOGI(TAG, "Phát hiện camera (kiểu OV2640) PID=0x%04X (địa chỉ=0x%02X)",
                             detected_pid, addr);
                    camera_found = true;
                ...
                if (ret == ESP_OK) {
                    ...
                    if (ret == ESP_OK) {
                        ...
                        if (detected_pid != 0) {
                            ESP_LOGI(TAG, "Phát hiện camera (kiểu OV3660) PID=0x%04X (địa chỉ=0x%02X)",
                                     detected_pid, addr);
                            camera_found = true;
                            i2c_master_bus_rm_device(dev_handle);
                            break;
        ...
        if (!camera_found) {
            ...
            // Xác định loại camera dựa trên PID
            if (detected_pid == OV2640_PID_1 || detected_pid == OV2640_PID_2) {
                camera_type_ = OTTO_CAMERA_OV2640;
                ESP_LOGI(TAG, "Loại camera: OV2640 (PID=0x%04X)", detected_pid);
            } else if (detected_pid == OV3660_PID) {
                camera_type_ = OTTO_CAMERA_OV3660;
                ESP_LOGI(TAG, "Loại camera: OV3660 (PID=0x%04X)", detected_pid);
            } else {
                camera_type_ = OTTO_CAMERA_UNKNOWN;
                ESP_LOGW(TAG, "Loại camera không xác định, PID=0x%04X", detected_pid);
            }
        }
        return camera_found;
    }
...
private:
    ...
    bool InitializeCamera() {
        ...
        try {
            ...
            // Đặt tham số lật ảnh khác nhau tùy theo loại camera
            switch (camera_type_) {
                case OTTO_CAMERA_OV3660:
                    camera_->SetVFlip(true);
                    camera_->SetHMirror(true);
                    ESP_LOGI(TAG, "OV3660: Đặt VFlip=true, HMirror=true");
                    break;
                case OTTO_CAMERA_OV2640:
                default:
                    camera_->SetVFlip(true);
                    camera_->SetHMirror(false);
                    ESP_LOGI(TAG, "OV2640: Đặt VFlip=true, HMirror=false");
                    break;
            }
            return true;
        } catch (...) {
...
public:
    OttoRobot()
        ...
        // Tự động phát hiện phiên bản phần cứng (đồng thời phát hiện loại camera)
        has_camera_ = DetectHardwareVersion();
        ESP_LOGI(TAG, "Tự động phát hiện phiên bản phần cứng: %s", has_camera_ ? "Có camera" : "Không có camera");
#elif OTTO_HARDWARE_VERSION == OTTO_VERSION_CAMERA
        // Bắt buộc dùng phiên bản có camera, nhưng vẫn phát hiện loại camera cụ thể
        has_camera_ = DetectHardwareVersion();
        if (!has_camera_) {
            // Phát hiện thất bại nhưng vẫn dùng cấu hình camera, không biết loại cụ thể
            has_camera_ = true;
            camera_type_ = OTTO_CAMERA_UNKNOWN;
            ESP_LOGW(TAG, "Bắt buộc dùng cấu hình có camera, nhưng không phát hiện được loại camera");
            // Khởi tạo bus I2C để dùng cho camera
            i2c_master_bus_config_t i2c_bus_cfg = {
                .i2c_port = I2C_NUM_0,
                .sda_io_num = CAMERA_VERSION_CONFIG.i2c_sda_pin,
                .scl_io_num = CAMERA_VERSION_CONFIG.i2c_scl_pin,
            ...
            ESP_LOGI(TAG, "Bắt buộc dùng cấu hình có camera");
        }
#elif OTTO_HARDWARE_VERSION == OTTO_VERSION_NO_CAMERA
        // Bắt buộc dùng phiên bản không có camera
        has_camera_ = false;
        camera_type_ = OTTO_CAMERA_NONE;
        ESP_LOGI(TAG, "Bắt buộc dùng cấu hình không có camera");
#else
#error \
    "OTTO_HARDWARE_VERSION không hợp lệ, hãy dùng OTTO_VERSION_AUTO, OTTO_VERSION_CAMERA hoặc OTTO_VERSION_NO_CAMERA"
#endif
