...
public:
    ...
    void RegisterMcpTools() {
        ...
        mcp_server.AddTool("self.otto.action",
                           ...
                           "Động tác cơ bản: walk(đi bộ, cần steps/speed/direction/arm_swing), turn(quay, cần steps/speed/direction/arm_swing), jump(nhảy, cần steps/speed),"
                           "swing(lắc lư, cần steps/speed/amount), moonwalk(bước trăng, cần steps/speed/direction/amount), bend(cúi người, cần steps/speed/direction),"
                           "shake_leg(rung chân, cần steps/speed/direction), updown(lên xuống, cần steps/speed/amount), whirlwind_leg(chân lốc xoáy, cần steps/speed/amount);"
                           "Động tác cố định: sit(ngồi), showcase(biểu diễn), home(về vị trí gốc);"
                           "Động tác tay (cần servo tay): hands_up(giơ tay, cần speed/direction), hands_down(hạ tay, cần speed/direction), hand_wave(vẫy tay, cần direction),"
                           "windmill(cánh quạt, cần steps/speed/amount), takeoff(cất cánh, cần steps/speed/amount), fitness(tập thể dục, cần steps/speed/amount),"
                           "greeting(chào hỏi, cần direction/steps), shy(xấu hổ, cần direction/steps), radio_calisthenics(thể dục buổi sáng), magic_circle(xoay vòng phép màu)",
                           PropertyList({
                           ...
                           [this](const PropertyList& properties) -> ReturnValue {
                               ...
                               // Động tác tay
                                   ...
                                   return "Lỗi: Tên động tác không hợp lệ. Các động tác hỗ trợ: walk, turn, jump, swing, moonwalk, bend, shake_leg, updown, whirlwind_leg, sit, showcase, home, hands_up, hands_down, hand_wave, windmill, takeoff, fitness, greeting, shy, radio_calisthenics, magic_circle";
                               }
                           });
