...
//--  Tham số:
//--    steps: Số bước nhún
//--    period: Chu kỳ (ms), nhỏ hơn = nhanh hơn
//--    height: Biên độ nhún hông (10-35)
//---------------------------------------------------------
void Otto::Samba(float steps, int period, int height) {
    // Samba: đùi luân phiên kết hợp bàn chân lệch pha +45°
    // tạo nhún hông đặc trưng; hai tay vung ngược chiều nhau theo nhịp
    int A[SERVO_COUNT] = {height, height, height, height, 25, 25};
    int O[SERVO_COUNT] = {0, 0, 5, -5, HAND_HOME_POSITION, 180 - HAND_HOME_POSITION};
    double phase_diff[SERVO_COUNT] = {
        DEG2RAD(-90), DEG2RAD(90),   // Đùi luân phiên
        DEG2RAD(-45), DEG2RAD(135),  // Bàn chân lệch +45° so với đùi → hông nhún đặc trưng Samba
        DEG2RAD(90),  DEG2RAD(-90)   // Tay vung ngược chiều nhau
    };
    Execute(A, O, period, phase_diff, steps);
}
...
//--  Tham số:
//--    steps: Số bước
//--    period: Chu kỳ (ms), nên dùng chậm 1000-1400
//--    height: Biên độ nhón chân (15-30)
//---------------------------------------------------------
void Otto::Ballet(float steps, int period, int height) {
    // Ballet: bàn chân nhón cao luân phiên; đùi gần đứng yên;
    // hai tay giơ cao thanh lịch và vẫy uyển chuyển đối nhau
    if (!has_hands_) {
        TiptoeSwing(steps, period, height);
        return;
    }
    int A[SERVO_COUNT] = {0, 0, height, height, 30, 30};
    int O[SERVO_COUNT] = {0, 0, height, -height, 130, 50};  // Tay giơ cao (130 ≈ 3/4 lên)
    double phase_diff[SERVO_COUNT] = {
        DEG2RAD(0), DEG2RAD(0),     // Đùi đứng yên
        DEG2RAD(0), DEG2RAD(180),   // Bàn chân luân phiên nhẹ nhàng
        DEG2RAD(0), DEG2RAD(180)    // Tay vẫy uyển chuyển đối nhau
    };
    Execute(A, O, period, phase_diff, steps);
}
...
//--    steps: Số bước lắc
//--    period: Chu kỳ (ms)
//--    height: Biên độ lắc hông (20-40)
//---------------------------------------------------------
void Otto::Hula(float steps, int period, int height) {
    // Hula Hawaii: bàn chân đồng pha → lắc hông cùng chiều liên tục;
    // đùi đứng yên; tay nâng nhẹ ngang eo, đung đưa ngược chiều hông
    if (!has_hands_) {
        Swing(steps, period, height);
        return;
    }
    int A[SERVO_COUNT] = {0, 0, height, height, 20, 20};
    int O[SERVO_COUNT] = {0, 0, height / 2, -(height / 2), 110, 70};  // Tay nâng nhẹ ~ngang eo
    double phase_diff[SERVO_COUNT] = {
        DEG2RAD(0), DEG2RAD(0),    // Đùi đứng yên
        DEG2RAD(0), DEG2RAD(0),    // Bàn chân ĐỒNG PHA → lắc hông cùng chiều
        DEG2RAD(90), DEG2RAD(-90)  // Tay đung đưa ngược chiều hông
    };
    Execute(A, O, period, phase_diff, steps);
}
...
//--  Tham số:
//--    steps: Số bước xoắn
//--    period: Chu kỳ (ms), nên dùng nhanh 500-700
//--    height: Biên độ xoắn chân (15-30)
//---------------------------------------------------------
void Otto::Twist(float steps, int period, int height) {
    // Twist: bàn chân lệch pha 180° tạo hiệu ứng xoắn thân người;
    // đùi dao động nhẹ đối pha; hai tay mở rộng vung đối nhau
    if (!has_hands_) {
        Jitter(steps, period, height);
        return;
    }
    int A[SERVO_COUNT] = {10, 10, height, height, 35, 35};
    int O[SERVO_COUNT] = {0, 0, 0, 0, HAND_HOME_POSITION, 180 - HAND_HOME_POSITION};
    double phase_diff[SERVO_COUNT] = {
        DEG2RAD(0),   DEG2RAD(180),  // Đùi đối pha, dao động nhẹ
        DEG2RAD(0),   DEG2RAD(180),  // Bàn chân LỆCH PHA 180° → xoắn người
        DEG2RAD(-90), DEG2RAD(90)    // Tay vung rộng đối nhau
    };
    Execute(A, O, period, phase_diff, steps);
}

//---------------------------------------------------------
//-- Điệu nhảy: Robot Dance
//--  Tham số:
//--    steps: Số lần lặp trái-phải
//--    period: Chu kỳ mỗi tư thế (ms)
//--    height: Góc nghiêng bàn chân (10-25)
//---------------------------------------------------------
void Otto::RobotDance(float steps, int period, int height) {
    // Điệu Robot: dùng MoveServos để tạo chuyển động giật cục cứng nhắc.
    // Luân phiên: bàn chân nghiêng phải + tay phải lên ↔ bàn chân nghiêng trái + tay trái lên
    if (!has_hands_) {
        Jitter(steps, period, height);
        return;
    }

    // Tư thế A: bàn chân nghiêng về phải, tay phải giơ cao
    // LEFT_FOOT → 0 (lên), RIGHT_FOOT → 0 (xuống, nghịch đảo) = nghiêng phải
    int pos_a[SERVO_COUNT] = {90, 90, 90 - height, 90 - height,
                              HAND_HOME_POSITION, 0};

    // Tư thế B: bàn chân nghiêng về trái, tay trái giơ cao
    // LEFT_FOOT → 180 (xuống), RIGHT_FOOT → 180 (lên, nghịch đảo) = nghiêng trái
    int pos_b[SERVO_COUNT] = {90, 90, 90 + height, 90 + height,
                              180, 180 - HAND_HOME_POSITION};

    // Tư thế home để kết thúc
    int pos_h[SERVO_COUNT] = {90, 90, 90, 90,
                              HAND_HOME_POSITION, 180 - HAND_HOME_POSITION};

    int half = period / 2;
    for (int i = 0; i < (int)steps; i++) {
        MoveServos(half, pos_a);
        MoveServos(half, pos_b);
    }
    MoveServos(half, pos_h);
}
