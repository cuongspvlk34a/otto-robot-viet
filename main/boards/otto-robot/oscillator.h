//--------------------------------------------------------------
//-- Oscillator.pde
//-- Tạo dao động hình sin cho các servo
//--------------------------------------------------------------
...
#define SERVO_TIMEBASE_RESOLUTION_HZ 1000000  // 1MHz, 1us mỗi tick
...
private:
    ...
    double phase0_;           //-- Pha ban đầu (radian)

    //-- Biến nội bộ
    int pos_;                       //-- Vị trí hiện tại của servo
    int pin_;                       //-- Chân kết nối servo
    int trim_;                      //-- Độ lệch hiệu chỉnh
    double phase_;                  //-- Pha hiện tại
    double inc_;                    //-- Bước tăng pha
    ...
    //-- Chế độ dao động. Nếu true, servo dừng lại
    bool stop_;
